...
final class StatisticsViewController<T>: BaseController, UISearchResultsUpdating, UISearchControllerDelegate, UISearchBarDelegate, UIConfigurable where T: Mappable {
    ...
    func setUI() {
        ...
        animation.autoPinEdgesToSuperviewEdges()
    ...
    func updateSearchResults(for searchController: UISearchController) {
        guard let search = searchController.searchBar.text, search.count >= 3 else { return }
        guard let searchViewController = searchController.searchResultsController as? SearchViewController else { return }
        workItem?.cancel()
        
        let workItem = DispatchWorkItem { [weak self] in
            self?.performSearch(search, withSearchController: searchViewController)
        }
    ...
    @objc func performSearch(_ search: String, withSearchController searchViewController: SearchViewController) {
    ...
    private func layout() -> UICollectionViewCompositionalLayout {
        ...
        let layout = UICollectionViewCompositionalLayout(sectionProvider: { [weak self] index, layoutEnvironment in
            ...
            case .warning:
                ...
                let groupSize = NSCollectionLayoutSize(widthDimension: .absolute(layoutWidth), heightDimension: .absolute(layoutWidth < 375 ? 48 : 28))
            ...
            case .firstDivider, .secondDivider, .thirdDivider, .fourtDivider:
                ...
                layoutSection = NSCollectionLayoutSection(group: group)
            default:
                let firstItemSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(2.2/3), heightDimension: .fractionalHeight(1))
                let firstItem = NSCollectionLayoutItem(layoutSize: firstItemSize)
                
                let secondItemSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(0.8/3), heightDimension: .fractionalHeight(1))
                let secondItem = NSCollectionLayoutItem(layoutSize: secondItemSize)
